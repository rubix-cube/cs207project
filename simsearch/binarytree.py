...
class BinaryTree(object):
    ...
    def get_closer_than(self, key):
        ...
        def inner(self, node, m, res):
            ...
            if node and node.hasLeftChild():
                new_node = self._follow(node.left_ref)
                inner(self, new_node, m, res)
            if node and node.hasRightChild():
                new_node = self._follow(node.right_ref)
                inner(self, new_node, m, res)
    
        if not self._storage.locked:
            self._refresh_tree_ref()
        node = self._follow(self._tree_ref)
        inner(self, node, key, res)
...
class DBDB(object):
    ...
    def get_closer_than(self, key):
        self._assert_not_closed
        return self._tree.get_closer_than(key)
